...
public class RunExample {
    ...
    public static void perform(Dao ratingsDao) {
        ...
        // execute db query on time series table with partition key day (-> 2014-06-29, 2014-06-30, ...) 
        ImmutableList<Object> daysToQuery  = ImmutableList.of(LocalDate.of(2014, 6, 29).toEpochDay(),
                                                              LocalDate.of(2014, 6, 30).toEpochDay());
        Publisher<HotelRating> ratingPublisher = ratingsDao.readSequenceWithKeys("epoch_day", daysToQuery)
                                                           .asEntity(HotelRating.class)
                                                           .executeRx();
        
        Observable<HotelRating> ratingStream = RxReactiveStreams.toObservable(ratingPublisher);
